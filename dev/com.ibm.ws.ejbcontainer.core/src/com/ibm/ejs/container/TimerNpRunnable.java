    ...
    public void run()
    {
        ...
        } catch (Throwable ex) // F743-425.CodRev
        {
            ...
            if (isTraceOn && tc.isDebugEnabled())
                Tr.debug(tc, "NP Timer failed : " + ex.getClass().getName() +
                             ":" + ex.getMessage(), ex);
                ...
                (ivRetries >= ivRetryLimit)) // and retry limit reached
            {
                ...
                // Note2: we're leaving the timer in a defunct state.  It will never fire again, but
                // we're not destroying it, in case the user wants to query timers in diagnosing retry failures
