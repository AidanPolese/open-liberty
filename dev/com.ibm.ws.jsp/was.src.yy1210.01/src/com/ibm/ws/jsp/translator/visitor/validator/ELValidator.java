...
//  defect PK84568 2009/04/23	Array index out of bounds in getELExpressions if '$' is last char of a CDATA section
//  defect PM16142 2010/07/15   sartoris   Remove else statement as valid deferred expressions with functions are not being fully parsed / validated 
...
public class ELValidator { 
    ...
    private static void validateElFunction(Element jspElement, String expression, ValidateResult result, ClassLoader loader, JspConfiguration jspConfiguration, HashMap prefixToUriMap ) throws JspCoreException {
        ...
        if (el.containsEL() && !jspConfiguration.elIgnored()
	        ...
	        ctx.setFunctionMapper(getFunctionMapper(el, loader, result));
	
	        try {
	        	validateEL(jspConfiguration.getExpressionFactory(), ctx, el, expression);            	
	        } catch (ELException e) {
    ...
    private static void validateEL(ExpressionFactory ef, ELContext ctx, ELNode.Nodes el, String value)
			throws ELException {
		if (el != null) {
			// determine exact type
			ValueExpression ve = ef.createValueExpression(ctx, value,
					String.class);
		}
    }
    ...
    private static FunctionMapper getFunctionMapper(ELNode.Nodes el, ClassLoader loader, ValidateResult result)
			throws JspTranslationException {
