...
// Defect 704526  Only remove the custom property in this file for PM10362	- pmdinh  06/13/2011
//
...
public abstract class AbstractJSPExtensionProcessor extends com.ibm.ws.webcontainer.extension.WebExtensionProcessor {
    ...
    public AbstractJSPExtensionProcessor(IServletContext webapp, 
        ...
        collabHelper = webapp.getCollaboratorHelper();
        ...
        try {
            ...
            //  defect jdkcompiler    
            String docRoot = webapp.getRealPath("/");
            ...
            if (jspOptions.isUseInMemory()) {
                ...
                JspInputSourceFactory jspInputSourceFactory = new JspInputSourceFactoryImpl(docRoot,contextURL, dru, false, jspClassloaderContext.getClassLoader(),webapp);
                JspTranslationEnvironmentImpl jspEnvironment = new JspTranslationEnvironmentImpl(jspOptions.getOutputDir().getPath(), webapp.getContextPath(), jspInputSourceFactory,
                        new JspResourcesFactoryImpl(jspOptions, context), jspClassloaderContext, jspCompilerFactory);
            ...
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable(Level.FINE)) {
                logger.logp(Level.FINE, CLASS_NAME, "JSPExtensionProcessor", "TagLibraryCache created in " + (end - start) + " ms");
            }
...
/*
        ...
        if (this.jspOptions.isPrepareJSPsSet()) {
            ...
            try {
                ...
                PrepareJspHelper pretouchHelper = new PrepareJspHelper(this, webapp, this.jspOptions);
...
*/
    ...
    public void handleRequest(ServletRequest req, ServletResponse res) throws Exception {
    	...
    	if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable(Level.FINE)) {
            logger.logp(Level.FINE, CLASS_NAME, "handleRequest", "handleRequest of AbstractJSPExtensionProcessor should never be called.  There must have been a translation exception.");
        }
    }
...
//Begin PK13570    WITH SYNCTOOSTHREAD ENABLED, SIMPLE FILES (.GIF, .HTML, ETC...)
//        String fileSystemPath = context.getRealPath(path);
//        if (fileSystemPath.endsWith("jar") || fileSystemPath.endsWith("zip")) { // extended
//                                                                                // document
//                                                                                // root
//                                                                                // archive
//                                                                                // check
//            returnValue = true; // archived resources are case sensitive
//                                // already.
//        } else {
//End PK13570    WITH SYNCTOOSTHREAD ENABLED, SIMPLE FILES (.GIF, .HTML, ETC...)
            // Begin 247773, check if we are on z and do SyncToOsThread handling
            // if so
            if (jspOptions.isZOS()) {
                returnValue = processZOSCaseCheck(path, checkWEBINF); //PK81387 - added checkWEBINF param
                /*
                ...
                try {
                    ...
                    if (returnValue && com.ibm.ws.util.FileSystem.isCaseInsensitive) {
                        returnValue = com.ibm.ws.util.FileSystem.uriCaseCheck(caseFile, path.toString());
                    }
                } finally {
                ...
                */
				...
				//End PK13570    WITH SYNCTOOSTHREAD ENABLED, SIMPLE FILES (.GIF, .HTML, ETC...)
					...
					//PM10362  Start
					if (!isValidFilePath(fileSystemPath)) {    //704526																			   
						if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE)){
    ...
    protected JspTranslationContext loadTranslationContext(String translationContextClass, ServletContext servletContext, String outputDir, String contextRoot)
        ...
        if (System.getSecurityManager() != null) {
                ...
                ctxt = (JspTranslationContext) AccessController.doPrivileged(new java.security.PrivilegedExceptionAction() {
                    public Object run() throws JspCoreException {
                        try {
                            ...
                            JspInputSourceFactory jspInputSourceFactory = new JspInputSourceFactoryImpl(docRoot,contextURL, null, false, jspClassloaderContext
                            ...
                            JspTranslationEnvironmentImpl jspEnvironment = new JspTranslationEnvironmentImpl(finalOutputDir, finalContextRoot,
                                    jspInputSourceFactory, new JspResourcesFactoryImpl(jspOptions, ctxt), jspClassloaderContext, jspCompilerFactory);
            ...
            try {
                ...
                URL contextURL = new File(docRoot).toURL();
                JspInputSourceFactory jspInputSourceFactory = new JspInputSourceFactoryImpl(docRoot,contextURL, null, false, jspClassloaderContext.getClassLoader(),servletContext);
                ...
                JspTranslationEnvironmentImpl jspEnvironment = new JspTranslationEnvironmentImpl(outputDir, contextRoot, jspInputSourceFactory,
                        new JspResourcesFactoryImpl(jspOptions, ctxt), jspClassloaderContext, jspCompilerFactory);
                ctxt.setJspTranslationEnviroment(jspEnvironment);
            } catch (Exception e) {
    ...
    protected boolean isValidFilePath(String filePath) {
        //704526
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable(Level.FINE)) {            
            logger.entering(CLASS_NAME," isValidFilePath ", " , filePath -> " + filePath);
        } //704526
        ...
        //704526
	    boolean returnValue = true;				//704526	
        ...
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable(Level.FINE)) 
			logger.exiting( CLASS_NAME, "isValidFilePath", " -> "+ returnValue);
