    ...
    public List generateSetters() throws JspCoreException {
        ...
        for (Iterator itr = jspId.getAttrNameList().iterator(); itr.hasNext();) {
            ...
            if (attrName.indexOf(':') != -1) {
                if (element.lookupNamespaceURI(attrName.substring(0, attrName.indexOf(':')))==null) {
                    //If a dynamic attribute has a prefix that doesn’t map to a namespace, a translation error must occur.
                    throw new JspTranslationException("jsp.error.dynamicAttributes.translationException", new Object[] { attrName });
                }
                attr = element.getAttributeNodeNS(attrName.substring(0, attrName.indexOf(':')), 
            ...
            if (attr.getName().equals("jsp:id") == false && attr.getName().startsWith("xmlns") == false) {
                ...
                String evalAttrValue = evaluateAttribute(attr.getName(), attr.getValue(), isDynamic, false, false, tai);
        		if(com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable(Level.FINEST)){
