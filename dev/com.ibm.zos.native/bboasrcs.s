...
*-----------------------------------------------------------
*
         TITLE 'BBOASRCS - WAS Connectors API Receive Req Spec-31bit'
BBOASRCS CSECT
BBOASRCS AMODE ANY
BBOASRCS RMODE ANY
         J     Around
         DC    CL8'BBOASRCS'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_RECVSPC_PARMS,R1
         LLGT  R9,BBOA1RCS_CONNHDL load connection hdl, clear high
         STG   R9,WOLA_CLIENT_RECVSPC_HANDLE store as 1st parm
         LLGT  R9,BBOA1RCS_SERVNAME load service name address
         STG   R9,WOLA_CLIENT_RECVSPC_SERVNAME store as 2nd parm
         LLGT  R9,BBOA1RCS_SERVNAMEL load service name len address
         STG   R9,WOLA_CLIENT_RECVSPC_SERVNAMEL store as 3rd parm
         LLGT  R9,BBOA1RCS_REQDATAL load request data len address
         STG   R9,WOLA_CLIENT_RECVSPC_DATAL store as 4th parm
         LLGT  R9,BBOA1RCS_ASYNC load async setting
         STG   R9,WOLA_CLIENT_RECVSPC_ASYNC store as 5th parm
         XC    WOLA_CLIENT_RECVSPC_CICSPRM,WOLA_CLIENT_RECVSPC_CICSPRM
         LLGT  R9,BBOA1RCS_RC   load return code address
         STG   R9,WOLA_CLIENT_RECVSPC_RC store as 7th parm
         LLGT  R9,BBOA1RCS_RSN  load reason code address
         STG   R9,WOLA_CLIENT_RECVSPC_RSN store as 8th parm
         DROP  R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTRCS)  call metalC clientRcvReqSpec routine
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1RCS_RC    Load address of BBOA1RCS RC
         LHI   R14,BBOAAPI_RC_ERROR8
         C     R14,0(R9)         Is it RC=8
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1RCS_RSN   Load address of BBOA1RCS RSN
...
GETTWAS  DS    0H
         ...
         L     R15,AVECTWAS_RRQS@  Load get data function
         DROP  R7
...
R15      EQU   15
         LTORG
         DS    0F
BBOASRCS@ DC   A(BBOASRCS)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTRCS  ALIAS C'ClientReceiveRequestSpecific'
           EXTRN CLNTRCS
