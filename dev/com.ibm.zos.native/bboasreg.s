...
*-----------------------------------------------------------
*
         TITLE 'BBOASREG - WAS Connectors API Register Service-31 bit'
BBOASREG CSECT
BBOASREG AMODE ANY
BBOASREG RMODE ANY
         J     Around
         DC    CL8'BBOASREG'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_REGISTER_PARMS,R1
         LLGT  R9,BBOA1REG_GROUP   load domain name, clear high halves
         STG   R9,WOLA_CLIENT_REGISTER_GROUP  store as 1st parm
         LLGT  R9,BBOA1REG_NODE    load node name address
         STG   R9,WOLA_CLIENT_REGISTER_WOLA2  store as 2nd parm
         LLGT  R9,BBOA1REG_SERVER  load server name address
         STG   R9,WOLA_CLIENT_REGISTER_WOLA3  store as 3rd parm
         LLGT  R9,BBOA1REG_REGNAME load region name address
         STG   R9,WOLA_CLIENT_REGISTER_NAME   store as 4th parm
         LLGT  R9,BBOA1REG_MINCONN   load min connection address
         STG   R9,WOLA_CLIENT_REGISTER_MINCONN store as 5th parm
         LLGT  R9,BBOA1REG_MAXCONN   load max connection address
         STG   R9,WOLA_CLIENT_REGISTER_MAXCONN store as 6th parm
         LLGT  R9,BBOA1REG_REGFLAGS  load register flags address
         STG   R9,WOLA_CLIENT_REGISTER_FLAGS  store as 7th parm
         XC    WOLA_CLIENT_REGISTER_CICSPRM,WOLA_CLIENT_REGISTER_CICSPRX
               M 8th parm is CICS parms - null in batch.
         LLGT  R9,BBOA1REG_RC   load return code address
         STG   R9,WOLA_CLIENT_REGISTER_RC     store as 9th parm
         LLGT  R9,BBOA1REG_RSN  load reason code address
         STG   R9,WOLA_CLIENT_REGISTER_RSN    store as 10th parm
         DROP  R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTREG)  call metalC clientRegister routine
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1REG_RC    Load address of BBOA1REG RC
         LHI   R14,BBOAAPI_RC_SEVERE12
         C     R14,0(R9)         Is it RC=12
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1REG_RSN   Load address of BBOA1REG RSN
         LHI   R14,BBOAAPI_RSN_BADBGVT
         CH    R14,2(R9)         Can't find the Liberty WOLA group?
...
*
         ...
         JZ    UNWIND            If not found, just return.
         USING BBOAMCSS,R7
...
R15      EQU   15
         LTORG
         DS    0F
BBOASREG@ DC   A(BBOASREG)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTREG  ALIAS C'ClientRegister'
           EXTRN CLNTREG
