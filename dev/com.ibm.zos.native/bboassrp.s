...
*-----------------------------------------------------------
*
         TITLE 'BBOASSRP - WAS Connectors API Send Response -31 bit'
BBOASSRP CSECT
BBOASSRP AMODE ANY
BBOASSRP RMODE ANY
         J     Around
         DC    CL8'BBOASSRP'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_SENDRESP_PARMS,R1
         LLGT  R9,BBOA1SRP_CONNHDL load connhandle address
         STG   R9,WOLA_CLIENT_SENDRESP_HANDLE store as 1st parm
*        rspdata comes in from client as addr of addr
*        we are saving the final address of the rspdata
*        in the parmlist so clientSendResponse routine takes a ptr
         LLGT  R6,BBOA1SRP_RSPDATA load resp data addr of addr
         LLGT  R9,0(,R6)        load resp data address
         STG   R9,WOLA_CLIENT_SENDRESP_DATA store as 2nd parm
         LLGT  R9,BBOA1SRP_RSPDATAL load request data len address
         STG   R9,WOLA_CLIENT_SENDRESP_DATAL store as 3rd parm
         XC    WOLA_CLIENT_SENDRESP_CICSPRM,WOLA_CLIENT_SENDRESP_CICSPRX
               M 4th parm is CICS struct - set to NULL
         LLGT  R9,BBOA1SRP_RC   load return code address
         STG   R9,WOLA_CLIENT_SENDRESP_RC store as 5th parm
         LLGT  R9,BBOA1SRP_RSN  load reason code address
         STG   R9,WOLA_CLIENT_SENDRESP_RSN store as 6th parm
         DROP  R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTSRP)  call metalC clientSendResp routine
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1SRP_RC    Load address of BBOA1SRP RC
         LHI   R14,BBOAAPI_RC_ERROR8
         C     R14,0(R9)         Is it RC=8
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1SRP_RSN   Load address of BBOA1SRP RSN
...
GETTWAS  DS    0H
         ...
         L     R15,AVECTWAS_SNRP@  Load send resp function
         DROP  R7
...
R15      EQU   15
         LTORG
         DS    0F
BBOASSRP@ DC   A(BBOASSRP)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTSRP  ALIAS C'ClientSendResponse'
           EXTRN CLNTSRP
