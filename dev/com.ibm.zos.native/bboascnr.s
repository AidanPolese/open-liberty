...
*-----------------------------------------------------------
*
         TITLE 'BBOASCNR - WAS Connectors API Connection Rel-31 bit'
BBOASCNR CSECT
BBOASCNR AMODE ANY
BBOASCNR RMODE ANY
         J     Around
         DC    CL8'BBOASCNR'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_CONNREL_PARMS,R1
         LLGT  R9,BBOA1CNR_CONNHDL   load connhandle address
         STG   R9,WOLA_CLIENT_CONNREL_HANDLE store as 1st parm
         XC    WOLA_CLIENT_CONNREL_CICSPRM,WOLA_CLIENT_CONNREL_CICSPRM
         LLGT  R9,BBOA1CNR_RC        load return code address
         STG   R9,WOLA_CLIENT_CONNREL_RC     store as 3rd parm
         LLGT  R9,BBOA1CNR_RSN       load reason code address
         STG   R9,WOLA_CLIENT_CONNREL_RSN    store as 4th parm
         DROP  R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTCNR)  call metalC clientConnectionRelease
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1CNR_RC    Load address of BBOA1CNR RC
         LHI   R14,BBOAAPI_RC_ERROR8
         C     R14,0(R9)         Is it RC=8
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1CNR_RSN   Load address of BBOA1CNR RSN
...
GETTWAS  DS    0H
         ...
         L     R15,AVECTWAS_CONR@  Load get data function
         DROP  R7
...
R15      EQU   15
         LTORG
         DS    0F
BBOASCNR@ DC   A(BBOASCNR)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTCNR  ALIAS C'ClientConnectionRelease'
           EXTRN CLNTCNR
