...
*-----------------------------------------------------------
*
         TITLE 'BBOASRCL - WAS Connectors API Recv Resp Len-31 bit'
BBOASRCL CSECT
BBOASRCL AMODE ANY
BBOASRCL RMODE ANY
         J     Around
         DC    CL8'BBOASRCL'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_RECVLEN_PARMS,R1
         LLGT  R9,BBOA1RCL_CONNHDL load connhandle addr
         STG   R9,WOLA_CLIENT_RECVLEN_HANDLE store as 1st parm
         LLGT  R9,BBOA1RCL_ASYNC load async setting addr
         STG   R9,WOLA_CLIENT_RECVLEN_ASYNC store as 2nd parm
         LLGT  R9,BBOA1RCL_RSPDATAL load resp data len address
         STG   R9,WOLA_CLIENT_RECVLEN_RSPDATAL store as 3rd parm
         XC    WOLA_CLIENT_RECVLEN_CICSPRM,WOLA_CLIENT_RECVLEN_CICSPRM
         LLGT  R9,BBOA1RCL_RC   load return code address
         STG   R9,WOLA_CLIENT_RECVLEN_RC store as 5th parm
         LLGT  R9,BBOA1RCL_RSN  load reason code address
         STG   R9,WOLA_CLIENT_RECVLEN_RSN store as 6th parm
         DROP  R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTRCL)  call metalC ClientRecvRespDataLen
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1RCL_RC    Load address of BBOA1RCL RC
         LHI   R14,BBOAAPI_RC_ERROR8
         C     R14,0(R9)         Is it RC=8
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1RCL_RSN   Load address of BBOA1RCL RSN
...
GETTWAS  DS    0H
         ...
         L     R15,AVECTWAS_RCRL@  Load get data function
         DROP  R7
...
R15      EQU   15
         LTORG
         DS    0F
BBOASRCL@ DC   A(BBOASRCL)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTRCL  ALIAS C'ClientReceiveResponseLength'
           EXTRN CLNTRCL
