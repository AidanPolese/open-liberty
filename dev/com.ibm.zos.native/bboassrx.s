...
*-----------------------------------------------------------
*
         TITLE 'BBOASSRX - WAS Connectors API Send Resp Excep-31 bit'
BBOASSRX CSECT
BBOASSRX AMODE ANY
BBOASSRX RMODE ANY
         J     Around
         DC    CL8'BBOASSRX'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_SENDEXCP_PARMS,R1
         LLGT  R9,BBOA1SRX_CONNHDL load connhandle addr
         STG   R9,WOLA_CLIENT_SENDEXCP_HANDLE store as 1st parm
...
*        in the parmlist so clientSendResponse routine takes a ptr
         LLGT  R6,BBOA1SRX_RSPDATA load resp data addr of addr
         LLGT  R9,0(,R6)        load resp data address
         STG   R9,WOLA_CLIENT_SENDEXCP_DATA store as 2nd parm
         LLGT  R9,BBOA1SRX_RSPDATAL load resp data len address
         STG   R9,WOLA_CLIENT_SENDEXCP_DATAL store as 3rd parm
         XC  WOLA_CLIENT_SENDEXCP_CICSPRM,WOLA_CLIENT_SENDEXCP_CICSPRM
         LLGT  R9,BBOA1SRX_RC   load return code address
         STG   R9,WOLA_CLIENT_SENDEXCP_RC store as 5th parm
         LLGT  R9,BBOA1SRX_RSN  load reason code address
         STG   R9,WOLA_CLIENT_SENDEXCP_RSN store as 6th parm
         DROP  R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTSRX)  call metalC ClientSendRespException
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1SRX_RC    Load address of BBOA1SRX RC
         LHI   R14,BBOAAPI_RC_ERROR8
         C     R14,0(R9)         Is it RC=8
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1SRX_RSN   Load address of BBOA1SRX RSN
...
GETTWAS  DS    0H
         ...
         L     R15,AVECTWAS_SNRX@  Load get data function
         DROP  R7
...
R15      EQU   15
         LTORG
         DS    0F
BBOASSRX@ DC   A(BBOASSRX)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTSRX  ALIAS C'ClientSendResponseException'
           EXTRN CLNTSRX
