...
*-----------------------------------------------------------
*
         TITLE 'BBOASSRQ - WAS Connectors API Send Request-31 bit'
BBOASSRQ CSECT
BBOASSRQ AMODE ANY
BBOASSRQ RMODE ANY
         J     Around
         DC    CL8'BBOASSRQ'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_SENDREQ_PARMS,R1
         LLGT  R9,BBOA1SRQ_CONNHDL load connection hdl, clear high
         STG   R9,WOLA_CLIENT_SENDREQ_HANDLE store as 1st parm
         LLGT  R9,BBOA1SRQ_REQTYPE load request type address
         STG   R9,WOLA_CLIENT_SENDREQ_REQTYPE store as 2nd parm
         LLGT  R9,BBOA1SRQ_SERVNAME load service name address
         STG   R9,WOLA_CLIENT_SENDREQ_SERVNAME store as 3rd parm
         LLGT  R9,BBOA1SRQ_SERVNAMEL load service name len address
         STG   R9,WOLA_CLIENT_SENDREQ_SERVNAMEL store as 4th parm
*        reqdata come in from client as addr of addr
*        we are saving the final address of the reqdata in the
*        parmlist so that ClientSendRequest routine takes a pointer
         LLGT  R6,BBOA1SRQ_REQDATA load request data addr of addr
         LLGT  R9,0(,R6)        load request data address
         STG   R9,WOLA_CLIENT_SENDREQ_REQDATA store as 5th parm
         LLGT  R9,BBOA1SRQ_REQDATAL load request data len address
         STG   R9,WOLA_CLIENT_SENDREQ_REQDATAL store as 6th parm
         LLGT  R9,BBOA1SRQ_ASYNC load async setting
         STG   R9,WOLA_CLIENT_SENDREQ_ASYNC store as 7th parm
         LLGT  R9,BBOA1SRQ_RSPDATAL load response data len address
         STG   R9,WOLA_CLIENT_SENDREQ_RSPDATAL store as 8th parm
         XC    WOLA_CLIENT_SENDREQ_CICSPRM,WOLA_CLIENT_SENDREQ_CICSPRM
         LLGT  R9,BBOA1SRQ_RC   load return code address
         STG   R9,WOLA_CLIENT_SENDREQ_RC store as 10th parm
         LLGT  R9,BBOA1SRQ_RSN  load reason code address
         STG   R9,WOLA_CLIENT_SENDREQ_RSN store as 11th parm
         DROP R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTSRQ)  call metalC ClientSendRequest
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1SRQ_RC    Load address of BBOA1SRQ RC
         LHI   R14,BBOAAPI_RC_ERROR8
         C     R14,0(R9)         Is it RC=8
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1SRQ_RSN   Load address of BBOA1SRQ RSN
...
GETTWAS  DS    0H
         ...
         BASR  R14,R15             Call tWAS send request function
         DROP  R3
*
...
R15      EQU   15
         LTORG
         DS    0F
BBOASSRQ@ DC   A(BBOASSRQ)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTSRQ  ALIAS C'ClientSendRequest'
           EXTRN CLNTSRQ
