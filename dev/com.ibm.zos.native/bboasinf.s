...
*-----------------------------------------------------------
*
         TITLE 'BBOASINF - WAS Connectors API Get Info-31 bit'
BBOASINF CSECT
BBOASINF AMODE ANY
BBOASINF RMODE ANY
         J     Around
         DC    CL8'BBOASINF'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_INFO_PARMS,R1
         LLGT  R9,BBOA1INF_REGNAME load register name, clear high
         STG   R9,WOLA_CLIENT_INFO_REGNAME store as 1st parm
         LLGT  R9,BBOA1INF_GROUP load group name address
         STG   R9,WOLA_CLIENT_INFO_WOLAGRP store as 2nd parm
         LLGT  R9,BBOA1INF_NODE load node name address
         STG   R9,WOLA_CLIENT_INFO_WOLA2 store as 3rd parm
         LLGT  R9,BBOA1INF_SERVER load server name address
         STG   R9,WOLA_CLIENT_INFO_WOLA3 store as 4th parm
         LLGT  R9,BBOA1INF_CONNDATA@ load registration details address@
         LLGT  R9,0(R9) load registration details address
         STG   R9,WOLA_CLIENT_INFO_CONNDATA store as 5th parm
         XC    WOLA_CLIENT_INFO_CICSPRM,WOLA_CLIENT_INFO_CICSPRM
         LLGT  R9,BBOA1INF_RC   load return code address
         STG   R9,WOLA_CLIENT_INFO_RC store as 7th parm
         LLGT  R9,BBOA1INF_RSN  load reason code address
         STG   R9,WOLA_CLIENT_INFO_RSN store as 8th parm
         DROP  R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTINF)  call metalC clientGetInfo routine
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1INF_RC    Load address of BBOA1INF RC
         LHI   R14,BBOAAPI_RC_ERROR8
         C     R14,0(R9)         Is it RC=8
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1INF_RSN   Load address of BBOA1INF RSN
...
R15      EQU   15
         LTORG
         DS    0F
BBOASINF@ DC   A(BBOASINF)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTINF  ALIAS C'ClientInfoGet'
           EXTRN CLNTINF
