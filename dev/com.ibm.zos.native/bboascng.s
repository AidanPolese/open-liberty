...
*-----------------------------------------------------------
*
         TITLE 'BBOASCNG - WAS Connectors API Connection Get-31 bit'
BBOASCNG CSECT
BBOASCNG AMODE ANY
BBOASCNG RMODE ANY
         J     Around
         DC    CL8'BBOASCNG'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_CONNGET_PARMS,R1
         LLGT  R9,BBOA1CNG_REGNAME load register name, clear high
         STG   R9,WOLA_CLIENT_CONNGET_NAME store as 1st parm
         LLGT  R9,BBOA1CNG_CONNHDL load connhandle address
         STG   R9,WOLA_CLIENT_CONNGET_HANDLE store as 2nd parm
         LLGT  R9,BBOA1CNG_WAITTIME load waittime address
         STG   R9,WOLA_CLIENT_CONNGET_WAITTIME store as 3rd parm
         XC    WOLA_CLIENT_CONNGET_CICSPRM,WOLA_CLIENT_CONNGET_CICSPRM
         LLGT  R9,BBOA1CNG_RC   load return code address
         STG   R9,WOLA_CLIENT_CONNGET_RC store as 5th parm
         LLGT  R9,BBOA1CNG_RSN  load reason code address
         STG   R9,WOLA_CLIENT_CONNGET_RSN store as 6th parm
         DROP  R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTCNG)  call metalC ClientConnectionGet
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1CNG_RC    Load address of BBOA1CNG RC
         LHI   R14,BBOAAPI_RC_ERROR8
         C     R14,0(R9)         Is it RC=8
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1CNG_RSN   Load address of BBOA1CNG RSN
...
GETTWAS  DS    0H
         ...
         L     R15,AVECTWAS_CONG@  Load get conn function
         DROP  R7
...
R15      EQU   15
         LTORG
         DS    0F
BBOASCNG@ DC   A(BBOASCNG)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTCNG  ALIAS C'ClientConnectionGet'
           EXTRN CLNTCNG
