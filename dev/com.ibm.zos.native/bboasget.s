...
*-----------------------------------------------------------
*
         TITLE 'BBOASGET - WAS Connectors API Get Data -31 bit'
BBOASGET CSECT
BBOASGET AMODE ANY
BBOASGET RMODE ANY
         J     Around
         DC    CL8'BBOASGET'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_GETDATA_PARMS,R1
         LLGT  R9,BBOA1GET_CONNHDL load connhandle addr
         STG   R9,WOLA_CLIENT_GETDATA_HANDLE store as 1st parm
* msgdata comes in from client as addr of addr
* we are saving the final address of the msgdata
* in the parmlist so clientGetData routine takes a ptr
         LLGT  R6,BBOA1GET_MSGDATA  load msg data addr of addr
         LLGT  R9,0(,R6)        load msg data address
         STG   R9,WOLA_CLIENT_GETDATA_DATA store as 2nd parm
         LLGT  R9,BBOA1GET_MSGDATAL load msg data len address
         STG   R9,WOLA_CLIENT_GETDATA_DATAL store as 3rd parm
         XC    WOLA_CLIENT_GETDATA_CICSPRM,WOLA_CLIENT_GETDATA_CICSPRM
         LLGT  R9,BBOA1GET_RC   load return code address
         STG   R9,WOLA_CLIENT_GETDATA_RC store as 5th parm
         LLGT  R9,BBOA1GET_RSN  load reason code address
         STG   R9,WOLA_CLIENT_GETDATA_RSN store as 6th parm
         LLGT  R9,BBOA1GET_RVAL load reason code address
         STG   R9,WOLA_CLIENT_GETDATA_RV store as 7th parm
         DROP  R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTGET)  call metalC ClientGetData
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1GET_RC    Load address of BBOA1GET RC
         LHI   R14,BBOAAPI_RC_ERROR8
         C     R14,0(R9)         Is it RC=8
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1GET_RSN   Load address of BBOA1GET RSN
...
R15      EQU   15
         LTORG
         DS    0F
BBOASGET@ DC   A(BBOASGET)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTGET  ALIAS C'ClientGetData'
           EXTRN CLNTGET
