...
*-----------------------------------------------------------
*
         TITLE 'BBOASGTX - WAS Connectors API Get Context-31 bit'
BBOASGTX CSECT
BBOASGTX AMODE ANY
BBOASGTX RMODE ANY
         J     Around
         DC    CL8'BBOASGTX'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_GETCTX_PARMS,R1
         LLGT  R9,BBOA1GTX_CONNHDL load connection hdl, clear high
         STG   R9,WOLA_CLIENT_GETCTX_HANDLE store as 1st parm
         LLGT  R6,BBOA1GTX_CTXTDATA load request data addr of addr
         LLGT  R9,0(,R6)        load request data address
         STG   R9,WOLA_CLIENT_GETCTX_DATA store as 2nd parm
         LLGT  R9,BBOA1GTX_CTXTLEN load service name address
         STG   R9,WOLA_CLIENT_GETCTX_DATAL store as 3rd parm
         XC    WOLA_CLIENT_GETCTX_CICSPRM,WOLA_CLIENT_GETCTX_CICSPRM
         LLGT  R9,BBOA1GTX_RC load service name len address
         STG   R9,WOLA_CLIENT_GETCTX_RC store as 5th parm
         LLGT  R9,BBOA1GTX_RSN load request data addr of addr
         STG   R9,WOLA_CLIENT_GETCTX_RSN store as 6th parm
         LLGT  R9,BBOA1GTX_RV load request data len address
         STG   R9,WOLA_CLIENT_GETCTX_RV store as 7th parm
         DROP  R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTGTX)  call metalC clientInvoke routine
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1GTX_RC    Load address of BBOA1GTX RC
         LHI   R14,BBOAAPI_RC_ERROR8
         C     R14,0(R9)         Is it RC=8
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1GTX_RSN   Load address of BBOA1GTX RSN
...
R15      EQU   15
         LTORG
         DS    0F
BBOASGTX@ DC   A(BBOASGTX)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTGTX  ALIAS C'ClientGetContext'
           EXTRN CLNTGTX
