...
/**
 * NOTE: this is a copy of CuTest.mc.  CuTest.mc is used for testing metal-C code
 *       whereas this copy is used for testing LE c++ code.
 */
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
    ...
    void * expected, void * actual, size_t len)
{
    ...
    CuStringAppend(&string, "expected <");
    CuStringAppendWithLen(&string, (const char *)expected, len);
    CuStringAppend(&string, "> but was <");
    CuStringAppendWithLen(&string, (const char *)actual, len);
    CuStringAppend(&string, ">");
    return CuFailInternal(tc, file, line, &string);
}
