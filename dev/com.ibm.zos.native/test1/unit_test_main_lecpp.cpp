...
extern CuSuite* jbatch_json_test_suite(void);
extern CuSuite* jbatch_native_utility_test_suite(void);
extern CuSuite* jbatch_utils_test_suite(void);
extern CuSuite* jbatch_models_test_suite(void);
...
CuSuite ** loadSuites(CuSuite ** suites) {
    ...
    suites[2] = jbatch_json_test_suite();
    suites[3] = jbatch_native_utility_test_suite();
    suites[4] = jbatch_utils_test_suite();
    suites[5] = jbatch_models_test_suite();
...
CuSuite * findSuite(CuSuite ** suites, const char * name) {
    for (int i = 0 ; i < NUM_SUITES; ++i) {
        ...
        if (suite != NULL && !strcmp(name, suite->name)) {
            return suite;
        }
    }
    return NULL;
...
int main(int argc, char** argv) {
    ...
    CuSuite * suites[ NUM_SUITES + 1 ];     // +1 cuz who cares.
    loadSuites(suites);
