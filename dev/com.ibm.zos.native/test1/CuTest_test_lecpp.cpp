...
/**
 * NOTE: this is a copy of CuTest_test.mc.  CuTest_test.mc is run with the 
 *       metal-C tests whereas this copy is run with the LE c++ tests.
 */
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
CuSuite* CuTest_test_string_suite(void)
{
    CuSuite* suite = CuSuiteNew("CuTest_test::CuStringGetSuite");
...
{
    CuSuite ts;
    CuSuiteInit(&ts, "suite1");
    CuAssertTrue(tc, ts.count == 0);
    CuAssertTrue(tc, ts.failCount == 0);
    CuAssertStrEquals(tc, "suite1", ts.name);
}

void TestCuSuiteNew(CuTest* tc)
{
    CuSuite* ts = CuSuiteNew("test suite");
    CuAssertTrue(tc, ts->count == 0);
    CuAssertTrue(tc, ts->failCount == 0);
    CuAssertStrEquals(tc, "test suite", ts->name);
}
...
{
    CuSuite* ts1 = CuSuiteNew("suite1");
    CuSuite* ts2 = CuSuiteNew("suite2");
...
/**
 * Helper method. Deliberate fail the test by calling CuFail.
 *
 * Note: when the assert fails it causes the method to return immediately.
 * Hence the need for a helper function in order to test the assert.
 */
void fail_CuFail(CuTest * tc, char * msg, int * pointReached) {
    CuFail(tc, "hello world");
    *pointReached = 1;
}
...
void TestFail(CuTest* tc)
{
    ...
    fail_CuFail(tc2, "hello world", &pointReached);

    CuAssert(tc, "point should not have been reached", pointReached == 0);
}


/**
 * Helper method.  Deliberately fail CuAssertStrEquals.  
 *
 * Note: when the assert fails it causes the method to return immediately.
 * Hence the need for a helper function in order to test the assert.
 */
void fail_CuAssertStrEquals(CuTest * tc, char * x, char * y) {
    CuAssertStrEquals(tc, x, y);
}

/**
 * Helper method.  Deliberately fail CuAssertStrEquals_Msg.  
 *
 * Note: when the assert fails it causes the method to return immediately.
 * Hence the need for a helper function in order to test the assert.
 */
void fail_CuAssertStrEquals_Msg(CuTest * tc, char * msg, char * x, char * y) {
    CuAssertStrEquals_Msg(tc, msg, x, y);
}
...
void TestAssertStrEquals(CuTest* tc)
{
    ...
    fail_CuAssertStrEquals(tc2, "hello", "world");

    CuAssertTrue(tc, tc2->failed);
    CompareAsserts(tc, "CuAssertStrEquals failed", expected, tc2->message);

    fail_CuAssertStrEquals_Msg(tc2, "some text", "hello", "world");
...
void TestAssertStrEquals_NULL(CuTest* tc)
{
    ...
    fail_CuAssertStrEquals(tc2, NULL, NULL);

    CuAssertTrue(tc, !tc2->failed);
    CompareAsserts(tc, "CuAssertStrEquals_NULL failed", NULL, tc2->message);

    fail_CuAssertStrEquals_Msg(tc2, "some text", NULL, NULL);
...
void TestAssertStrEquals_FailNULLStr(CuTest* tc)
{
    ...
    fail_CuAssertStrEquals(tc2, "hello", NULL);

    CuAssertTrue(tc, tc2->failed);
    CompareAsserts(tc, "CuAssertStrEquals_FailNULLStr failed", expected, tc2->message);

    fail_CuAssertStrEquals_Msg(tc2, "some text", "hello", NULL);
...
void TestAssertStrEquals_FailStrNULL(CuTest* tc)
{
    ...
    fail_CuAssertStrEquals(tc2, NULL, "hello");

    CuAssertTrue(tc, tc2->failed);
    CompareAsserts(tc, "CuAssertStrEquals_FailStrNULL failed", expected, tc2->message);

    fail_CuAssertStrEquals_Msg(tc2, "some text", NULL, "hello");
...
/**
 * Helper method.  Deliberately fail CuAssertIntEquals.  
 *
 * Note: when the assert fails it causes the method to return immediately.
 * Hence the need for a helper function in order to test the assert.
 */
void fail_CuAssertIntEquals(CuTest * tc, int x, int y) {
    CuAssertIntEquals(tc, x, y);
}

/**
 * Helper method.  Deliberately fail CuAssertIntEquals_Msg.  
 *
 * Note: when the assert fails it causes the method to return immediately.
 * Hence the need for a helper function in order to test the assert.
 */
void fail_CuAssertIntEquals_Msg(CuTest * tc, char * msg, int x, int y) {
    CuAssertIntEquals_Msg(tc, msg, x, y);
}
...
void TestAssertIntEquals(CuTest* tc)
{
    ...
    fail_CuAssertIntEquals(tc2, 42, 32);
    
    CuAssertTrue(tc, tc2->failed);
    CompareAsserts(tc, "CuAssertIntEquals failed", expected, tc2->message);
    
    fail_CuAssertIntEquals_Msg(tc2, "some text", 42, 32);
...
/**
 * Helper method.  Deliberately fail CuAssertDblEquals.  
 *
 * Note: when the assert fails it causes the method to return immediately.
 * Hence the need for a helper function in order to test the assert.
 */
void fail_CuAssertDblEquals(CuTest * tc, double x, double y, double delta) {
    CuAssertDblEquals(tc, x, y, delta);
}

/**
 * Helper method.  Deliberately fail CuAssertDblEquals_Msg.  
 *
 * Note: when the assert fails it causes the method to return immediately.
 * Hence the need for a helper function in order to test the assert.
 */
void fail_CuAssertDblEquals_Msg(CuTest * tc, char * msg, double x, double y, double delta) {
    CuAssertDblEquals_Msg(tc, msg, x, y, delta);
}
...
void TestAssertDblEquals(CuTest* tc)
{
    ...
    fail_CuAssertDblEquals(tc2, x, y, 0.001);
    
    CuAssertTrue(tc, tc2->failed);
    CompareAsserts(tc, "CuAssertDblEquals failed", expected, tc2->message);

    fail_CuAssertDblEquals_Msg(tc2, "some text", x, y, 0.001);
...
/**
 * Helper method.  Deliberately fail CuAssertMemEquals.  
 *
 * Note: when the assert fails it causes the method to return immediately.
 * Hence the need for a helper function in order to test the assert.
 */
void fail_CuAssertMemEquals(CuTest * tc, void * x, void * y, int len) {
    CuAssertMemEquals(tc, x, y, len);
}

void TestAssertMemEquals(CuTest* tc)
{
    CuTest *tc2 = CuTestNew("fail_TestAssertMemEquals", zTestFails);

    fail_CuAssertMemEquals(tc2, "hello", "world", 5);

    CuAssertTrue(tc, tc2->failed);

    CuTest *tc3 = CuTestNew("pass_TestAssertMemEquals", TestPasses);

    CuAssertMemEquals(tc3, "hello", "hello", 5);

    CuAssertFalse(tc, tc3->failed);
}

/**
 * Helper method.  Deliberately fail CuAssertMemBytesEqual
 *
 * Note: when the assert fails it causes the method to return immediately.
 * Hence the need for a helper function in order to test the assert.
 */
void fail_CuAssertMemBytesEqual(CuTest * tc, int x, void * y, int len) {
    CuAssertMemBytesEqual(tc, x, y, len);
}

void TestAssertMemBytesEqual(CuTest* tc)
{
    CuTest *tc2 = CuTestNew("fail_TestAssertMemBytesEqual", zTestFails);

    fail_CuAssertMemBytesEqual(tc2, 0, "world", 5);

    CuAssertTrue(tc, tc2->failed);

    CuTest *tc3 = CuTestNew("pass_TestAssertMemBytesEqual", TestPasses);

    char blah[5];
    memset(blah, 0, 5);

    CuAssertMemBytesEqual(tc3, 0, blah, 5);
...
CuSuite* CuTest_test_suite(void)
{
    CuSuite* suite = CuSuiteNew("CuTest_test::CuGetSuite");
    ...
    SUITE_ADD_TEST(suite, TestAssertMemEquals);
    SUITE_ADD_TEST(suite, TestAssertMemBytesEqual);
