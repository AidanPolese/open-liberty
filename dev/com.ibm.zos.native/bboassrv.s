...
*-----------------------------------------------------------
*
         TITLE 'BBOASSRV - WAS Connectors API Host Service-31 bit'
BBOASSRV CSECT
BBOASSRV AMODE ANY
BBOASSRV RMODE ANY
         J     Around
         DC    CL8'BBOASSRV'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_HOST_PARMS,R1
         LLGT  R9,BBOA1SRV_REGNAME load register name
         STG   R9,WOLA_CLIENT_HOST_REGNAME store as 1st parm
         LLGT  R9,BBOA1SRV_SERVNAME load service name address
         STG   R9,WOLA_CLIENT_HOST_SERVNAME store as 2nd parm
         LLGT  R9,BBOA1SRV_SERVNAMEL load service name len address
         STG   R9,WOLA_CLIENT_HOST_SERVNAMEL store as 3rd parm
*        reqdata comes in from client as addr of addr
*        we are saving the final address of the reqdata
*        in the parmlist so clientHostService routine takes a ptr
         LLGT  R6,BBOA1SRV_REQDATA load request data addr of addr
         LLGT  R9,0(,R6)        load request data address
         STG   R9,WOLA_CLIENT_HOST_REQDATA store as 4th parm
         LLGT  R9,BBOA1SRV_REQDATAL load request data len address
         STG   R9,WOLA_CLIENT_HOST_REQDATAL store as 5th parm
         LLGT  R9,BBOA1SRV_CONNHDL load connhandle address
         STG   R9,WOLA_CLIENT_HOST_HANDLE store as 6th parm
         LLGT  R9,BBOA1SRV_WAITTIME@ load waittime address
         STG   R9,WOLA_CLIENT_HOST_WAITTIME store as 7th parm
         XC    WOLA_CLIENT_HOST_CICSPRM,WOLA_CLIENT_HOST_CICSPRM
         LLGT  R9,BBOA1SRV_RC   load return code address
         STG   R9,WOLA_CLIENT_HOST_RC store as 9th parm
         LLGT  R9,BBOA1SRV_RSN  load reason code address
         STG   R9,WOLA_CLIENT_HOST_RSN store as 10th parm
         LLGT  R9,BBOA1SRV_RVAL load return value address
         STG   R9,WOLA_CLIENT_HOST_RV store as 11th parm
         DROP  R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTSRV)  call metalC clientHostService routine
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1SRV_RC    Load address of BBOA1SRV RC
         LHI   R14,BBOAAPI_RC_ERROR8
         C     R14,0(R9)         Is it RC=8
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1SRV_RSN   Load address of BBOA1SRV RSN
...
GETTWAS  DS    0H
         ...
         L     R15,AVECTWAS_HOST@  Load get data function
         DROP  R7
...
R15      EQU   15
         LTORG
         DS    0F
BBOASSRV@ DC   A(BBOASSRV)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTSRV  ALIAS C'ClientHostService'
           EXTRN CLNTSRV
