...
*-----------------------------------------------------------
*
         TITLE 'BBOASINV - WAS Connectors API Invoke Service-31 bit'
BBOASINV CSECT
BBOASINV AMODE ANY
BBOASINV RMODE ANY
         J     Around
         DC    CL8'BBOASINV'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_INVOKE_PARMS,R1
         LLGT  R9,BBOA1INV_REGNAME    load register name
         STG   R9,WOLA_CLIENT_INVOKE_REGNAME store as 1st parm
         LLGT  R9,BBOA1INV_REQTYPE    load request type address
         STG   R9,WOLA_CLIENT_INVOKE_REQTYPE store as 2nd parm
         LLGT  R9,BBOA1INV_SERVNAME   load service name address
         STG   R9,WOLA_CLIENT_INVOKE_SERVNAME store as 3rd parm
         LLGT  R9,BBOA1INV_SERVNAMEL  load service name len address
         STG   R9,WOLA_CLIENT_INVOKE_SERVNAMEL store as 4th parm
*        regdata and rspdata come in from client as addr of addr
*        we are saving the final address of the reqdata and respdata
*        in the parmlist so that clientInvoke routine takes a pointer
         LLGT  R6,BBOA1INV_REQDATA    load request data addr of addr
         LLGT  R9,0(,R6)        load request data address
         STG   R9,WOLA_CLIENT_INVOKE_REQDATA store as 5th parm
         LLGT  R9,BBOA1INV_REQDATAL   load request data len address
         STG   R9,WOLA_CLIENT_INVOKE_REQDATAL store as 6th parm
         LLGT  R6,BBOA1INV_RSPDATA    load response data addr of addr
         LLGT  R9,0(,R6)        load response data address
         STG   R9,WOLA_CLIENT_INVOKE_RSPDATA store as 7th parm
         LLGT  R9,BBOA1INV_RSPDATAL   load response data len address
         STG   R9,WOLA_CLIENT_INVOKE_RSPDATAL store as 8th parm
         LLGT  R9,BBOA1INV_WAITTIME@  load waittime address
         STG   R9,WOLA_CLIENT_INVOKE_WAITTIME store as 9th parm
         XC    WOLA_CLIENT_INVOKE_CICSPRM,WOLA_CLIENT_INVOKE_CICSPRM
         LLGT  R9,BBOA1INV_RC         load return code address
         STG   R9,WOLA_CLIENT_INVOKE_RC store as 11th parm
         LLGT  R9,BBOA1INV_RSN        load reason code address
         STG   R9,WOLA_CLIENT_INVOKE_RSN store as 12th parm
         LLGT  R9,BBOA1INV_RVAL       load return value address
         STG   R9,WOLA_CLIENT_INVOKE_RV  store as 13th parm
         DROP  R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTINV)  call metalC clientInvoke routine
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1INV_RC    Load address of BBOA1INV RC
         LHI   R14,BBOAAPI_RC_ERROR8
         C     R14,0(R9)         Is it RC=8
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1INV_RSN   Load address of BBOA1INV RSN
...
R15      EQU   15
         LTORG
         DS    0F
BBOASINV@ DC   A(BBOASINV)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTINV  ALIAS C'ClientInvoke'
           EXTRN CLNTINV
