...
*-----------------------------------------------------------
*
         TITLE 'BBOASURG - WAS Adapters API Register - 31 bit'
BBOASURG CSECT
BBOASURG AMODE ANY
BBOASURG RMODE ANY
         J     Around
         DC    CL8'BBOASURG'            Module name
...
*
         LA    R1,PLIST
         USING WOLA_CLIENT_UNREGISTER_PARMS,R1
         LLGT    R9,BBOA1URG_REGNAME load region name
         STG     R9,WOLA_CLIENT_UNREGISTER_NAME store as 1st parm
         LLGT    R9,BBOA1URG_URGFLAGS load unregister flags address
         STG     R9,WOLA_CLIENT_UNREGISTER_FLAGS store as 2nd parm
         XC      WOLA_CLIENT_UNREGISTER_CICSPRM,WOLA_CLIENT_UNREGISTER_X
               CICSPRM Clear 3rd parm - CICS parms
         LLGT    R9,BBOA1URG_RC   load return code address
         STG     R9,WOLA_CLIENT_UNREGISTER_RC store as 4th parm
         LLGT    R9,BBOA1URG_RSN  load reason code address
         STG     R9,WOLA_CLIENT_UNREGISTER_RSN store as 5th parm
         DROP  R1
*
* Make call to 64 bit metal C function.
*
         SAM64                  switch to 64 bit mode,
         LLGT  R15,=V(CLNTURG)  call metalC ClientUnRegister routine
         BASR  R14,R15
         SAM31
*
* See if we need to call the tWAS WOLA stub instead.
*
         L     R9,BBOA1URG_RC    Load address of BBOA1URG RC
         LHI   R14,BBOAAPI_RC_ERROR8
         C     R14,0(R9)         Is it RC=8
         JNZ   UNWIND            Not RC=8, continue
         L     R9,BBOA1URG_RSN   Load address of BBOA1URG RSN
...
GETTWAS  DS    0H
         ...
         L     R15,AVECTWAS_UREG@  Load get data function
         DROP  R7
...
R15      EQU   15
         LTORG
         DS    0F
BBOASURG@ DC   A(BBOASURG)         Address of this routine for BBOAICAL
IMSLIT   DC    CL4'BBOA'           IMS ESAF LIT for WOLA
IMSKey   DC    X'70'               IMS KEY 7
*
CLNTURG  ALIAS C'ClientUnRegister'
           EXTRN CLNTURG
