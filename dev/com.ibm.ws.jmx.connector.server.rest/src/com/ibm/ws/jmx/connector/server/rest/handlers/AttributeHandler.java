...
public class AttributeHandler implements RESTHandler {
    ...
    private void getAttributes(RESTRequest request, RESTResponse response) {
        ...
        if (RESTHelper.containsSingleRoutingContext(request)) {
            if (MBeanRouterHelper.getCachedAttribute(request, response, objectName, null, queryAttributes)) {
                //we were able to get the attribute from cache
                return;
            } else {
                try {
    ...
    private void getAttribute(RESTRequest request, RESTResponse response) {
        ...
        if (RESTHelper.containsSingleRoutingContext(request)) {
            if (MBeanRouterHelper.getCachedAttribute(request, response, objectName, attributeName, null)) {
                //we were able to get the attribute from cache
                return;
            } else {
                try {
