...
public class FileTransferHelper {
    ...
    private void downloadRXA(String downloadFrom, String downloadToDir, String targetHost, String targetServer, String targetUserDir) throws IOException {
        MBeanRouterHelper.getReader().receiveFile(targetHost, targetUserDir, targetServer, downloadFrom, new File(downloadToDir));
    }

    private void uploadRXA(String uploadFrom, String uploadToDir, String targetHost, String targetServer, String targetUserDir) throws IOException {
        MBeanRouterHelper.getReader().sendFile(targetHost, targetUserDir, targetServer, new File(uploadFrom), uploadToDir);
    }

    private void deleteRXA(String deleteFile, boolean recursiveDelete, String targetHost, String targetServer, String targetUserDir) throws IOException {
        RemoteAccessWrapper remoteAccess = null;
        try {
            CollectivePlugin plugin = MBeanRouterHelper.getReader();
            remoteAccess = plugin.createRemoteAccess(targetHost, targetUserDir, targetServer, null, null);
            plugin.deleteFile(remoteAccess, deleteFile, recursiveDelete);
        } finally {
    ...
    private boolean expandArchiveRXA(String targetPathDir, String archiveName, String archiveTargetPath, String targetHost, String targetServer,
                                     String targetUserDir) throws IOException {
        //Get the plugin
        final CollectivePlugin plugin = MBeanRouterHelper.getReader();

        final String archiveSourcePath = appendFilename(targetPathDir, archiveName);
        final String[] arguments = { "\"" + archiveSourcePath + "\"",
                                     "\"" + archiveTargetPath + "\"" };

        try {
            //We don't require the archive to be a Liberty archive
            plugin.expandArchive(targetHost, targetUserDir, targetServer, arguments, targetPathDir, false);
        } finally {
            //RXA delete archive (we either expanded the archive or got an error)
            plugin.deleteFile(targetHost, targetUserDir, targetServer, archiveSourcePath);
        }
    ...
    public String processRoutingPath(String originalPath, String targetHost, String targetServer, String targetUserDir) {
        ...
        try {
            //Resolve the symbol using Atlas
            String nodeValue = (String) MBeanRouterHelper.getReader().getServerNode(targetHost, targetUserDir, targetServer, "sys.paths/" + symbolToResolve.getName());
            if (nodeValue == null) {
                if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
                    Tr.debug(this, tc, "Node value was null");
                }
                //Node value was not found in Atlas.  This could happen if the server was registered as
                //a member but was never actually started (hence its paths weren't published).  Fallback
                //into a default calculation of the variables.
                if (targetUserDir.charAt(targetUserDir.length() - 1) != '/') {
                    targetUserDir = targetUserDir + "/";
                }
                nodeValue = symbolToResolve.getDefault(targetUserDir, targetServer);
            }

            //Return the replaced string
            return nodeValue != null ? path.replace(symbolToResolve.getSymbol(), nodeValue) : null;

        } catch (IOException e) {
            if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
                Tr.debug(this, tc, "Exception in FileTransferHelper: " + e.getMessage());
            }
            //Return null to let the caller handle it.
            return null;
        }
    }
    ...
    public void routedDeleteInternal(RESTRequest request, String filePath, boolean recursiveDelete) {
        ...
        //Process target path
        String deleteFile = processRoutingPath(filePath, targetHost, targetServer, targetUserDir);

        if (deleteFile == null) {
            IOException ioe = new IOException(TraceNLS.getFormattedMessage(this.getClass(),
                                                                           APIConstants.TRACE_BUNDLE_FILE_TRANSFER,
                                                                           "PATH_NOT_VALID",
                                                                           new String[] { decodeFilePath(filePath) },
                                                                           "CWWKX0127E: The path " + decodeFilePath(filePath) + " is not valid."));
            throw ErrorHelper.createRESTHandlerJsonException(ioe, null, APIConstants.STATUS_BAD_REQUEST);
        }

        try {
            //Call helper to delete targetPath (remote) using RXA
            deleteRXA(deleteFile, recursiveDelete, targetHost, targetServer, targetUserDir);
        } catch (IOException e) {
            throw ErrorHelper.createRESTHandlerJsonException(e, null, APIConstants.STATUS_INTERNAL_SERVER_ERROR);
        }

        if (TraceComponent.isAnyTracingEnabled() && tc.isInfoEnabled()) {
            Tr.info(tc, "DELETE_REQUEST_COMPLETE_INFO", deleteFile);
        }
    }
    ...
    public void routedUploadInternal(RESTRequest request, String filePath, boolean expansion, boolean legacyFileTransfer) {
        ...
        //Process target path
        String targetPath = processRoutingPath(filePath, targetHost, targetServer, targetUserDir);
        ...
        if (expansion) {
            fileName = FileTransferHelper.getTempArchiveName(request, fileName);
        }

        //Fetch target directory
        String uploadToDir = getParentDir(targetPath);

        //Get a unique writable location
        String uploadFrom = getWritableLocation();

        //Append the fileName
        uploadFrom = uploadFrom + fileName;

        //Read contents of stream into file
        readRequestIntoFile(uploadFrom, request, legacyFileTransfer);

        try {
            if (expansion) {
                uploadToDir = targetPath;
            }
            //Call helper to move file between uploadFrom (local) and uploadToDir_processed (remote) using RXA
            uploadRXA(uploadFrom, uploadToDir, targetHost, targetServer, targetUserDir);
        } catch (IOException e) {
            throw ErrorHelper.createRESTHandlerJsonException(e, null, APIConstants.STATUS_INTERNAL_SERVER_ERROR);
        } finally {
            //Delete temp file
            if (!deleteLocalFileAndParentDir(uploadFrom)) {
                //if we coudn't delete the temporary file or parent directory, log it but don't throw error, since the overall operation worked.
                if (TraceComponent.isAnyTracingEnabled() && tc.isEventEnabled()) {
                    Tr.event(this, tc, "Could not delete temporary file (or parent directory): " + uploadFrom);
                }
            }
        }
        ...
        if (expansion) {
            try {
                //Expand the remote archive using RXA
                expandArchiveRXA(uploadToDir, fileName, targetPath, targetHost, targetServer, targetUserDir);
            } catch (IOException e) {
                throw ErrorHelper.createRESTHandlerJsonException(e, null, APIConstants.STATUS_INTERNAL_SERVER_ERROR);
            }
        }
        ...
        if (TraceComponent.isAnyTracingEnabled() && tc.isInfoEnabled()) {
            Tr.info(tc, "UPLOAD_REQUEST_COMPLETE_INFO", targetPath);
        }
    ...
    public void routedDownloadInternal(RESTRequest request, RESTResponse response, String filePath, boolean legacyFileTransfer) {
        ...
        //Process target path
        String sourcePath = processRoutingPath(filePath, targetHost, targetServer, targetUserDir);
        ...
        //Get the writeList for this (Atlas) server, so that we know where we can move the file to
        String downloadToDir = getWritableLocation();

        try {
            //Call helper to move file between downloadFrom (remote) and downloadToDir (local) using RXA
            downloadRXA(sourcePath, downloadToDir, targetHost, targetServer, targetUserDir);
        } catch (IOException e) {
            throw ErrorHelper.createRESTHandlerJsonException(e, null, APIConstants.STATUS_INTERNAL_SERVER_ERROR);
        }

        //Write contents of file into returning stream
        final String tempFile = downloadToDir + fileName;
        writeResponseFromFile(tempFile, request, response, legacyFileTransfer);

        //Delete temp file
        if (!deleteLocalFileAndParentDir(tempFile)) {
            //if we coudn't delete the temporary file or parent directory, log it but don't throw error, since the overall operation worked.
            if (TraceComponent.isAnyTracingEnabled() && tc.isEventEnabled()) {
                Tr.event(this, tc, "Could not delete temporary file (or parent directory): " + tempFile);
            }
        }

        if (TraceComponent.isAnyTracingEnabled() && tc.isInfoEnabled()) {
            Tr.info(tc, "DOWNLOAD_REQUEST_COMPLETE_INFO", sourcePath);
        }
    }
}
