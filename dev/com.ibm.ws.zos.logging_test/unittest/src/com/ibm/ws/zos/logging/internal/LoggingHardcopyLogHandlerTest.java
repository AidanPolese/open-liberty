...
public class LoggingHardcopyLogHandlerTest {
    ...
    protected LoggingHardcopyLogHandler hardcopyLH = new LoggingHardcopyLogHandler() {
        @Override
        protected int ntv_WriteToOperatorProgrammerAndHardcopy(byte[] msg) {
            return mockHardcopyLH.ntv_WriteToOperatorProgrammerAndHardcopy(msg);
        }
    };
    ...
    public void test_LoggingHardcopyLogHandlerNull() {
        ...
        mockery.checking(new Expectations() {
            {
                oneOf(mockHardcopyLH).ntv_WriteToOperatorProgrammerAndHardcopy(with(equal(msgBytes)));
                will(returnValue(-102));
            }
        });

        int rc = hardcopyLH.ntv_WriteToOperatorProgrammerAndHardcopy(null);
        assertEquals(-102, rc);
    ...
    public void test_LoggingHardcopyLogHandlerPublishNullMessageAndLogRecord() {
        ...
        mockery.checking(new Expectations() {
            {
                never(mockHardcopyLH).ntv_WriteToOperatorProgrammerAndHardcopy(null);
            }
        });
    ...
    public void test_LoggingHardcopyLogHandlerGerman() {
        ...
        hardcopyLH = new LoggingHardcopyLogHandler() {
            @Override
            protected int ntv_WriteToOperatorProgrammerAndHardcopy(byte[] msg) {
                return mockHardcopyLH.ntv_WriteToOperatorProgrammerAndHardcopy(msg);
            }
