...
public class SRTServletResponse implements HttpServletResponse, IResponseOutput, IExtendedResponse, IServletResponse, Response, IOutputStreamObserver {
	...
	private static final boolean isArdConfigEnabled;
...
protected static Logger logger = LoggerFactory.getInstance().getLogger("com.ibm.ws.webcontainer.srt");
    ...
    //protected BufferedServletOutputStream _bufferedOut = new BufferedServletOutputStream(DEFAULT_BUFFER_SIZE);
	...
	static {
		if (com.ibm.wsspi.webcontainer.WebContainer.getWebContainer().getWebContainerConfig().isArdEnabled()) 
			isArdConfigEnabled = true;		
		else
			isArdConfigEnabled = false;
	}
    ...
    public String getHeader(String key) {
        //311717
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE)) {  //306998.15
            logger.logp(Level.FINE, CLASS_NAME,"getHeader", " name --> " + key + " response --> " + _response.getHeader(key),"["+this+"]");
        }
    ...
    public void setBufferSize(int size) {
        ...
        if (_responseBuffer != null) {
            if (!_firstWrite) {
                _responseBuffer.setBufferSize(size);
            }
            else {
    ...
    public int getBufferSize() {
        if (_responseBuffer == null) {
            ...
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE))  //306998.15
                logger.logp(Level.FINE, CLASS_NAME,"getBufferSize", "getBufferSize --> " + _responseBuffer.getBufferSize(),"["+this+"]");
            return _responseBuffer.getBufferSize();
        }
    }
    ...
    public void flushBuffer(boolean flushToWire) throws IOException {
        ...
        try {
            ...
            if (_gotOutputStream) {
                _responseBuffer.flushBuffer();
                commited = true;
            }
        	}

            if (!isCommitted() && !commited) {
            	if (isTraceOn&&logger.isLoggable (Level.FINE))
...
//    }
    ...
    public boolean isCommitted() {
        ...
        if (_gotWriter || _gotOutputStream) {
            if (_responseBuffer == null) {            	
            	if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE))
        	...
        	if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE))
                logger.logp(Level.FINE, CLASS_NAME, "isCommitted" , "responseBuffer isCommitted="+_responseBuffer.isCommitted(),"["+this+"]");

            return _responseBuffer.isCommitted();
        }
        else {
        	if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE))
    ...
    public ServletOutputStream getOutputStream() {
        ...
        _bufferedOut.init(_rawOut, getBufferSize());
        _bufferedOut.setLimit(_contentLength);
        _bufferedOut.setResponse(_response);
        _responseBuffer = _bufferedOut;
        } //PK89810 End
    ...
    * @return SRTConnectionContext
    ...
    public void addCookie(Cookie cookie) {
        ...
        if (dispatchContext.isInclude() == true) {
            ...
            if (!_ignoreStateErrors && isCommitted()) {
                ...
                logger.logp(Level.WARNING, CLASS_NAME,"addCookie", "Cannot.set.header.Response.already.committed");
            }
            else {
    ...
    public void sendRedirect(String location) throws IOException {
        ...
        if (dispatchContext.isInclude() == true) {
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE)) {  //306998.15
                logger.logp(Level.FINE, CLASS_NAME,"sendRedirect", nls.getString("Illegal.from.included.servlet", "Illegal from included servlet"), "sendRedirect location --> " + location);  //311717
            }
        }
        else {
    ...
    public void setContentLength(int len) {
        ...
        if (dispatchContext.isInclude() == true) {
            ...
            _bufferedOut.setLimit(_contentLength = len);
            _bufferedWriter.setLimit(_contentLength);
    ...
    public void setDateHeader(String name) {
        ...
        if (dispatchContext.isInclude() == true) {
            ...
            if (!_ignoreStateErrors && isCommitted()) {
                // log a warning...ignore headers set after response is committed
                logger.logp(Level.WARNING, CLASS_NAME,"setDateHeader", "Cannot.set.header.Response.already.committed");
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE)) {  //306998.15
    ...
    public void setHeader(String name, String s, boolean checkInclude) {
        ...
        if (checkInclude&&dispatchContext.isInclude() == true) {
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE)) {  //306998.15
                logger.logp(Level.FINE, CLASS_NAME,"setHeader", nls.getString("Illegal.from.included.servlet", "Illegal from included servlet"), "setHeader name --> " + name + " value --> " + s + " checkInclude --> " + checkInclude);  //311717
            }
        }
        else {
            ...
            if (!_ignoreStateErrors && isCommitted()) {
                // log a warning...ignore headers set after response is committed
                logger.logp(Level.WARNING, CLASS_NAME,"setHeader", "Cannot.set.header.Response.already.committed");
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE)) {  //306998.15
    ...
    public void setIntHeader(String name, int i) {
        ...
        if (dispatchContext.isInclude() == true) {
            ...
            if (!_ignoreStateErrors && isCommitted()) {
                // log a warning...ignore headers set after response is committed
                logger.logp(Level.WARNING, CLASS_NAME,"setIntHeader", "Cannot.set.header.Response.already.committed");
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE)) {  //306998.15
    ...
    public void setLocale(Locale loc) {
        ...
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE)) {  //306998.15
            logger.exiting(CLASS_NAME,"setLocale", "contentType = " + contentType);  //321485
        }
    }
    ...
    public void addDateHeader(String name, long date) {
        ...
        if (dispatchContext.isInclude() == true) {
            ...
            if (!_ignoreStateErrors && isCommitted()) {
                // log a warning...ignore headers set after response is committed
                logger.logp(Level.WARNING, CLASS_NAME,"addDateHeader", "Cannot.set.header.Response.already.committed");
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE))  //306998.15
    ...
    public void addHeader(String name, String value) {
        ...
        if (dispatchContext.isInclude() == true) {
            ...
            if (!_ignoreStateErrors && isCommitted()) {
                // log a warning...ignore headers set after response is committed
                logger.logp(Level.WARNING, CLASS_NAME,"addHeader", "Cannot.set.header.Response.already.committed");
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE))  //306998.15
    ...
    public void addIntHeader(String name, int value) {
        ...
        if (dispatchContext.isInclude() == true) {
            ...
            if (!_ignoreStateErrors && isCommitted()) {
                // log a warning...ignore headers set after response is committed
                logger.logp(Level.WARNING, CLASS_NAME,"addIntHeader", "Cannot.set.header.Response.already.committed");
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE))  //306998.15
    ...
    private void setCommonHeaders() {
    	...
    	if (!WCCustomProperties.DISABLE_X_POWERED_BY){
	    	if (WCCustomProperties.X_POWERED_BY==null){
	    		xPoweredByValue = WebContainerConstants.X_POWERED_BY_DEFAULT_VALUE;
	    	}
	    	else {
