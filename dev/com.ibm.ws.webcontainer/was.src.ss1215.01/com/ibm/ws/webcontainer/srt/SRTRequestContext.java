...
protected static Logger logger = LoggerFactory.getInstance().getLogger("com.ibm.ws.webcontainer.srt");
	...
	public void sessionPreInvoke(WebApp webapp)
	{
		...
		if (session == null)
		{
			...
			session = webapp.getSessionContext().sessionPreInvoke((HttpServletRequest) request, (HttpServletResponse) request.getResponse());
			if (session != null)
	...
	public HttpSession getSession(boolean create, WebApp webapp)
	{
		...
		if (session != null)
		{
		    ...
		    {
		        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE)) {
		            logger.logp(Level.FINE, CLASS_NAME,"getSession", "session is not valid, remove from webapp");
		        }

		        webappToSessionMap.remove(webapp);
		    }                                                           //PM87133 - end
		}
