...
public class MetaInfResourcesFileUtils {
    ...
    public MetaInfResourcesFileUtils(ServletContext ctxt) {
            ...
            if (libBinPathList==null||libBinPathList.isEmpty())
        	{
            	...
            	recurseDirs = true;
            	searchRoot = ((IServletContext)ctxt).getRealPath("/WEB-INF/lib","MetaInfRes");   // PM21451
            	File rootDir = new File(searchRoot);
        		libBinFiles = rootDir.listFiles();
        	} else {
        		if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE)) {
        		...
        		//if we've gotten the list from WCCM, there's no need to recurse as they've already identified everything
        		recurseDirs = false;
    ...
    public Set<String> getResourcePaths(File[] searchPath,String filename, Set setOfUrls){
    	if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE))
            logger.logp(Level.FINE, CLASS_NAME,"getResourcePaths", "searchPath-->"+searchPath+", filename --> " + filename);
