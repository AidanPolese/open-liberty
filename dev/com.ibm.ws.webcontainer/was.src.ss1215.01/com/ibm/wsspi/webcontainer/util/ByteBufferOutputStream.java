...
public class ByteBufferOutputStream extends WSServletOutputStream {
    ...
    public void write(int ch) throws IOException {
		if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE)) {  //306998.15
			logger.logp(Level.FINE, CLASS_NAME,"write", "write --> "+ch+", limit->"+limit);
        }
		if (limit > -1)
		{
			if (total >= limit)
    ...
    public void write(byte[] buf, int offset, int len) throws IOException {
		if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable (Level.FINE)) {  //306998.15
			logger.logp(Level.FINE, CLASS_NAME,"write", "write len --> "+len+", limit->"+limit);
        }
		if (len < 0)
		{
    ...
    private WsByteBuffer getNewByteBuffer() {
        return (WsByteBufferPoolManagerImpl.getRef().allocateDirect(bufferSize));
    }
