...
public abstract class CollaboratorHelper implements ICollaboratorHelper {
    ...
    public static EnumSet<CollaboratorInvocationEnum> allCollabEnum = EnumSet.allOf(CollaboratorInvocationEnum.class); // The
                                                                                                                       // default
                                                                                                                       // would
                                                                                                                       // be
                                                                                                                       // to
    ...
    public void preInvokeCollaborators(ICollaboratorMetaData collabMetaData, EnumSet<CollaboratorInvocationEnum> colEnum) throws ServletException,
        ...
        try {
            if (collabMetaData.getServletContext().getSessionContext() != null) {
                sessionSecurityIntegrationEnabled = collabMetaData.getServletContext().getSessionContext().getIntegrateWASSecurity();
            ...
            if (colEnum != null && colEnum.contains(CollaboratorInvocationEnum.CLASSLOADER)) {
                ...
                if (warClassLoader != origClassLoader) {
                    if (logger.isLoggable(Level.FINE))
                        logger.logp(Level.FINE, CLASS_NAME, "handleRequest", "PK26183 re-set class loader from --> " + origClassLoader.toString()
