...
public class StoredResponse extends HttpServletResponseWrapper implements StoredResponseCompat, HttpServletResponse, Serializable, IResponseOutput, IExtendedResponse, IOutputStreamObserver
{
    ...
    private List<com.ibm.wsspi.buffermgmt.WsByteBuffer> _bbList = null;
	...
	public StoredResponse(IExtendedResponse extResponse,boolean isInclude) { //delegate everything but output stream/writer?
	...
	private void initOutputStream() {
		_bbOut = new ByteBufferOutputStream();
        _out = new SRTOutputStream();
        _out.init(_bbOut);
        _bbOut.setObserver(this);
	}
    ...
    public ServletOutputStream getOutputStream() throws IllegalStateException, IOException 
    {
        ...
        this.fireOutputStreamRetrievedEvent(_out);
        return _out;
    }
    ...
    public void setBufferSize(int size) throws IllegalStateException //ARD: how do we keep this in sync with top levle writer
    {
        ...
        if (_bbOut!=null){ // Call reset instead of flushing the SRTOutputStream which commits the response
        	_bbOut.reset();
        }
    ...
    public PrintWriter getWriter() throws IOException, IllegalStateException
    {
        ...
        try
        {
            ...
            if (encoding == null)
            {
                _outWriter = new OutputStreamWriter(_out);
            }
            else
            {
                _outWriter = new OutputStreamWriter(_out, encoding);
            }
        }
    ...
    protected void finishOutput(boolean toByteArray) throws IOException{
    	if (!finishedOutput){
	        ...
	        if (_out!=null){
		        ...
		        _bbOut.flushBuffer();
		        if (toByteArray) {
		        	_outputBuffer = _bbOut.toByteArray();
		        	_bbOut.reset();
		        }
		        else {
		        	_bbList = _bbOut.getByteBufferList();
		        }
	        }
    	}
    }
...
//            _outputStreamObtained = false;
    ...
    public List<com.ibm.wsspi.buffermgmt.WsByteBuffer> getByteBufferList() throws IOException{
    	finishOutput(false);
    	return _bbList;
    }
...
//    		InnerEnumeration innerEnum = new InnerEnumeration(_header.getHeaderNames(),headerTable[0].elements());
...
//	}
    ...
    public String getCharacterEncoding()
    {
    	if (!dummyResponse)
    		...
    		return _encoding; //don't need to check dummy response because it is kept up to date locally
    }
     ...
     * This method returns the date value specified in the last-modified header.
     */
     ...
     * This method is useful for caches to determine if the
     * response is up-to-date or if if it should be cleared from
     * the cache.
     * This method returns false by default, subclasses should
     * overload this method to provide their own expiration
     * policies.
     */
     ...
     * This method is optimized to quickly transfer the contents of this
     * response into another response. This method is useful when this
     * response is cached to generate the same response later.
     */
    public void transferResponse(HttpServletResponse target) throws IOException //never called for ARD. would be in trouble with a bunch of include warnings
    {
        _finish();
        ...
        else
        {
            ...
            if (this.getOutputBuffer()!= null) {   // PM17019  
	            //transfer data
    ...
    {
        // just need to clear the internal buffer since this response cannot be committed
    	if (_writer!=null)
    		_writer.flush();
        if (_bbOut!=null)
        	this._bbOut.reset();
