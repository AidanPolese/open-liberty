...
/**
 * WebSphere implementation of the OSGi log service:
 * One of these per bundle (via TrLogServiceFactory).
 * 
 * This logs directly via Tr before publishing the log entry to
 * other readers/listeners
 */
public class TrLogServiceImpl implements LogService {
    ...
    TrLogServiceImpl(TrLogImpl logImpl, Bundle b) {
        ...
        if (symName.startsWith("org.eclipse.equinox.metatype")) {
            ...
            if (group == null) {
                tc = new OSGiTraceComponent(logName, this.getClass(), new String[] { symName, OsgiLogConstants.LOG_SERVICE_GROUP });
            } else {
                tc = new OSGiTraceComponent(logName, this.getClass(), new String[] { symName, group, OsgiLogConstants.LOG_SERVICE_GROUP });
            }
        }
