...
public class DataSourceService extends AbstractConnectionFactoryService implements AppDefinedResource, ResourceFactory, ApplicationRecycleComponent {
    ...
    /**
     * TODO: Beta implementation of Testable.test for DataSource. Replace this comment once the official SPI is introduced.
     * 
     * @param props detail on how to test. Includes <ul>
     * <li>auth=application|container|[unspecified]
     * <li>authAlias
     * <li>user
     * <li>password
     * </ul>
     ...
     * a the "failure" key must be included, with the value either being a Throwable or a String indicating the error.
     */
    public LinkedHashMap<String, ?> test(@Sensitive Map<String, Object> props) {
        ...
        try {
            ...
            if (authType >= 0) {
                ResourceConfigFactory configFactory = PrivHelper.getService(componentContext, ResourceConfigFactory.class);
                config = configFactory.createResourceConfig(DataSource.class.getName());
                config.setResAuthType(authType);
                if (authAlias != null)
                    config.addLoginProperty("DefaultPrincipalMapping", authAlias); // set provided auth alias                
            }

            DataSource ds = (DataSource) createResource(config);
