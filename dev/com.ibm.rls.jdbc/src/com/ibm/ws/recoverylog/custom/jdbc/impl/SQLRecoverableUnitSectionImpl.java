 ...
 */
  ...
  */
  ...
  {
    if (tc.isEntryEnabled()) Tr.entry(tc, "SQLRecoverableUnitSectionImpl",new java.lang.Object[] {recLog,recUnit, new Long(recoverableUnitIdentity),new Integer(identity),new Boolean (singleData)});
    ...
    if (tc.isEntryEnabled()) Tr.exit(tc, "SQLRecoverableUnitSectionImpl", this);
  }
  ...
  public void addData(byte[] data) throws InternalLogException
  {
    ...
    if (_singleData)
    {
        ...
        { 
            ...
            if (tc.isDebugEnabled()) Tr.debug(tc, "There is existing WRITTEN data.");
        ...
        { 
            ...
            if (tc.isDebugEnabled()) Tr.debug(tc, "There is existing UNWRITTEN data.");
        ...
        else
        {
            ...
            if (tc.isDebugEnabled()) Tr.debug(tc, "There is no existing data.");
...
// recovery method to add data directly to _writtenData array
      ...
      try
      {
          if (_singleData)
          {
              if (_singleDataUpdated)
                  _recLog.updateRUSection(_recoverableUnitIdentity , _identity, data);
              else
                  _recLog.writeRUSection(_recoverableUnitIdentity , _identity, index, data);
          }
          else
          {
              _recLog.writeRUSection(_recoverableUnitIdentity , _identity, 1 + index, data);
          }
      }
  ...
  */
  public byte[] lastData()
  {
    ...
    if (tc.isEntryEnabled()) Tr.exit(tc, "lastData",RLSUtils.toHexString(_lastDataItem,RLSUtils.MAX_DISPLAY_BYTES));
