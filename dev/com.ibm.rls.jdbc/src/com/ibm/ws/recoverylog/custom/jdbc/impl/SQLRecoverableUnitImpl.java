...
import com.ibm.ws.recoverylog.spi.*;
...
public class SQLRecoverableUnitImpl implements RecoverableUnit
{
  ...
  * </p>
  */
  private java.util.HashMap _recoverableUnitSections;
  ...
  */
  ...
  {
    if (tc.isEntryEnabled()) Tr.entry(tc, "SQLRecoverableUnitImpl",new java.lang.Object[]{recLog, new Long(identity), failureScope, new Boolean(recovered)});
    ...
    if (tc.isEntryEnabled()) Tr.exit(tc, "SQLRecoverableUnitImpl", this);
  }
  ...
  SQLRecoverableUnitImpl(SQLMultiScopeRecoveryLog recLog, long identity, FailureScope failureScope)
  {
    ...
    if (tc.isEntryEnabled()) Tr.entry(tc, "SQLRecoverableUnitImpl", new Object[] {recLog, new Long(identity), failureScope});
    if (tc.isEntryEnabled()) Tr.exit(tc, "SQLRecoverableUnitImpl", this);
  }
  ...
  */
  ...
  {
    if (tc.isEntryEnabled()) Tr.entry(tc, "createSection",new java.lang.Object[] {this,new Integer(identity),new Boolean(singleData)});

    // If the parent recovery log instance has experienced a serious internal error then prevent
    // this operation from executing.
    if (_recLog.failed())
    {
      if (tc.isEntryEnabled()) Tr.exit(tc, "createSection",this);
    ...
    if (_recoverableUnitSections.containsKey(sectionId) == false)
    {
      ...
      if (tc.isEventEnabled()) Tr.event(tc,"SQLRecoverableUnitImpl '" + _identity + "' created a new RecoverableUnitSection with identity '" + identity + "'");
    }
    else
    {
      if (tc.isEventEnabled()) Tr.event(tc,"SQLRecoverableUnitImpl '" + _identity + "' was unable to create a RecoverableUnitSection with id '" + identity + "' as it already exists");      

      if (tc.isEntryEnabled()) Tr.exit(tc, "createSection", "RecoverableUnitSectionExistsException");
      throw new RecoverableUnitSectionExistsException(null);
    }
    
    if (tc.isEntryEnabled()) Tr.exit(tc, "createSection",recoverableUnitSection);
  ...
  */
  ...
  {
    if (tc.isEntryEnabled()) Tr.entry(tc, "removeSection",new java.lang.Object[]{this,new Integer(identity)});

    // REQD: Implementation not yet provided. No users of the RLS currently require this operation.

    if (tc.isEntryEnabled()) Tr.exit(tc, "removeSection", "UnsupportedOperationException");
  ...
  */
  ...
  {
    if (tc.isEntryEnabled()) Tr.entry(tc, "writeSection",new java.lang.Object[]{this,target});

    // If the parent recovery log instance has experienced a serious internal error then prevent
    // this operation from executing.
    if (_recLog.failed())
    {
      if (tc.isEntryEnabled()) Tr.exit(tc, "writeSection",this);
      throw new InternalLogException(null);
    }

    target.write();

    if (tc.isEntryEnabled()) Tr.exit(tc, "writeSection");
  }
  ...
  */
  ...
  {
    if (tc.isEntryEnabled()) Tr.entry(tc, "forceSections",new java.lang.Object[]{this,new Boolean(rewriteRequired)});

    // If the parent recovery log instance has experienced a serious internal error then prevent
    // this operation from executing.
    if (_recLog.failed())
    {
      if (tc.isEntryEnabled()) Tr.exit(tc, "forceSections",this);
      throw new InternalLogException(null);
    }
    ...
    catch(InternalLogException exc)
    {
      ...
      if (tc.isEntryEnabled()) Tr.exit(tc, "forceSections", exc);
      throw exc;
    }
    catch(Throwable exc)
    {
      FFDCFilter.processException(exc, "com.ibm.ws.recoverylog.spi.SQLRecoverableUnitImpl.forceSections", "537", this);
      if (tc.isEntryEnabled()) Tr.exit(tc, "forceSections", "InternalLogException");
      throw new InternalLogException(exc);
    }

    if (tc.isEntryEnabled()) Tr.exit(tc, "forceSections");
  }
  ...
  */
  ...
  {
    if (tc.isEntryEnabled()) Tr.entry(tc, "sections",this);
    ...
    if (tc.isEntryEnabled()) Tr.exit(tc, "sections",cursor);
  ...
  */
  public long identity()
  {
    ...
    if (tc.isEntryEnabled()) Tr.exit(tc, "identity",new Long(_identity));
    return _identity;
  }
  ...
  */
  void remove() throws InternalLogException
  {
    ...
    if (_recLog.failed())
    {
      if (tc.isEntryEnabled()) Tr.exit(tc, "remove",this);
      throw new InternalLogException(null);
    }
    ...
    if (tc.isEventEnabled()) Tr.event(tc, "Remove completed for recoverable unit " + _identity + ". Clear internal state");
  ...
  {
      if (tc.isEntryEnabled()) Tr.entry(tc, "failureScope", this);
      if (tc.isEntryEnabled()) Tr.exit(tc, "failureScope", _failureScope);
