...
/**
 * This class is used to set the Comment attribute of the cookie.
 * 
 */
public class CookieCommentData extends CookieData {

    /** Trace component for debugging */
    private static final TraceComponent tc = Tr.register(CookieCommentData.class, HttpMessages.HTTP_TRACE_NAME, HttpMessages.HTTP_BUNDLE);

    /**
     * Constructor for a cookie comment object.
     */
    ...
    public boolean set(HttpCookie cookie, byte[] attribValue) {
        if (null != attribValue && 0 < attribValue.length) {
            cookie.setComment(HttpChannelUtils.getEnglishString(attribValue));
            if (TraceComponent.isAnyTracingEnabled() && tc.isDebugEnabled()) {
                Tr.debug(tc, "Cookie Comment set to " + cookie.getComment());
            }
            return true;
        }
        return false;
