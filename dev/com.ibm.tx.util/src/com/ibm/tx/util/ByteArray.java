...
// --------  ----------    ------   -----------
// 07/08/06  johawkes      451213.1 Ripped off from WAS
...
public class ByteArray
{
    ...
    public ByteArray(byte[] d)
    {
        ...
        // By default, hashcode is over the entire byte array.             d181754
        computeHashCode(0);
    }
    ...
    protected ByteArray(ByteArray b)
    {
        ...
        System.arraycopy(b.data, 0, data, 0, b.data.length);
        hashcode = b.hashcode;
        hashcode32 = b.hashcode32;                                      // d119287
    }
    ...
    /**
     * Get hashcode for this ByteArray; the hashcode is computed once
     * and cached
     */
    ...
    /**
     * Returns true iff given object is a ByteArray that wraps
     * identical data. <p>
     *
    ...
    {
        if (this == o)       // Optimize 'self' comparison.             d181754
            return true;

        if (  ! (o instanceof ByteArray) ) //d211567
            return false;

        final ByteArray array = (ByteArray) o;

        if (hashcode != array.hashcode) {
            return false;
        }
        ...
        // Start at the end of the array, since that is where
        // the primary key is... and the most likely differences.       d120577
        int i = data.length-1;
        while (data[i] == array.data[i] && --i >= 0) {}
    ...
    {
        if (this == array)      // Optimize 'self' comparison.
            return true;

        if (array == null || hashcode != array.hashcode) {              // d273615
            return false;
        }
        ...
        // Start at the end of the array, since that is where
        // the primary key is... and the most likely differences.
        int i = data.length-1;
        while (data[i] == array.data[i] && --i >= 0) {}
    ...
    /**
     * Return a String representation of this ByteArray; converts each
     * byte into a pair of hexadecimal characters
     */
    public String toString()
    {
        ...
        for (int i = 0; i < data.length; i++) {
            result.append(HEX_CHARS[(data[i] >> 4) & 0xF]);
            result.append(HEX_CHARS[data[i] & 0xF]);
        }
...
//  --------------------------------------------------------------------------
    ...
    /**
     * Internal method that allows subclasses to update the wrappered byte array
     * and have the hashcode recomputed. <p>
     *
     * This implementation calculates a hashcode that based on all bytes in
     * the byte array. <p>
     *
     * @param data  new byte array to be wrappered.
     **/
    protected final void updateBytes(byte[] data)
    {
        this.data = data;
        computeHashCode(0);
    }
     ...
     * and have the hashcode recomputed. <p>
     *
     * This implementation calculates a hashcode that is based on the bytes
     * starting at the specified offset through the end of the byte array. <p>
     *
     * @param data        new byte array to be wrappered.
     * @param hashOffset  starting offset to begin hashcode calculation;
     *                    all prior bytes will be ignored for hashcode.
     **/                                                               // d181754
    protected final void updateBytes(byte[] data, int hashOffset)
    {
        this.data = data;
        computeHashCode(hashOffset);
    }
     ...
     * and hashcode. <p>
     *
     * This implementation uses the specified pre-calculated hashcode that is
     * based on the bytes starting at the specified offset through the end of
     * the byte array. <p>
     *
     * @param data        new byte array to be wrappered.
     * @param hashOffset  starting offset to begin hashcode calculation;
     *                    all prior bytes will be ignored for hashcode.
     * @param hashCode    pre-computed hashcode, based on hashOffset.
     **/                                                               // d181754
    protected final void updateBytes(byte[] data, int hashOffset, int hashCode)
    {
        this.data = data;
        hashcode = hashCode;
        hashcode32 = hashCode;
    ...
    /**
     * Tests if two ByteArray regions are equal. <p>
     *
     * A subregion of this ByteArray object is compared to a subregion of
     * the argument array.  The result is true if these subregions represent
     * identical byte sequences.  The subregion of the ByteArrays to compare
     * begins at the index offset and extend to the end of the ByteArrays. <p>
     *
     * False is returned if the byte arrays are not the same length. <p>
     *
     * @param offset starting offset of the subregion to compare.
     * @param array the ByteArray argument to compare.
     **/                                                               // d181754
    protected final boolean regionMatches(int offset, ByteArray array)
    {
        if (array == null || data.length != array.data.length) {        // d273615
            return false;
        }
        ...
        // the primary key is... and the most likely differences.
        int i = data.length-1;
        while (data[i] == array.data[i] && --i >= offset) {}
     ...
     * Department University of Auckland, Auckland, New Zealand (1996). <p>
     *
     * @param hashOffset  starting offset to begin hashcode calculation;
     *                    all prior bytes will be ignored for hashcode.
     */                                   // Added hashOffset parameter - d181754
    private final void computeHashCode(int hashOffset)
    {
...
//  Computed hashcode for the array
    private long hashcode;
    private int hashcode32;                                            // d119287
