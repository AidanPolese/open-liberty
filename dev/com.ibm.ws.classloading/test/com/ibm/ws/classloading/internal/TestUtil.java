...
final class TestUtil {
    ...
    public static URL getOtherClassesURL(boolean isChild) throws MalformedURLException {
        URL loc = getTestClassesURL();
        return new URL(loc.toString().replaceAll("_test/.*", "_other/build/" + isChild + "/"));
    }

    public static TestUtilClassLoader getLoaderFor(boolean isChild) throws MalformedURLException {
        URL[] urls = { getOtherClassesURL(isChild) };
    ...
    public static URL getServletJarURL() throws MalformedURLException, FileNotFoundException {
        String servletJarLocation = System.getProperty(SERVLET_JAR_LOCATION);
        File servletJar = new File(servletJarLocation);
        // check it exists!
