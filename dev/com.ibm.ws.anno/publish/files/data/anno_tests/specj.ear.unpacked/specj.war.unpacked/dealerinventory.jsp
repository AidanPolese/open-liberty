...
<%@ page import="java.util.Collection, 
          ...
          isThreadSafe="true" 
          isErrorPage="false"%>
<jsp:useBean id="inventory"  type="java.util.Collection<CustomerInventory>" scope="request"/>
<jsp:useBean id="openOrders" type="java.util.List<Order>" scope="request"/>
<jsp:useBean id="specUtils"  class="org.spec.jappserver.servlet.helper.SpecUtils" 
                             scope="session"/>
...
%>
<%
	Iterator<CustomerInventory> invItr = inventory.iterator();
	int numVehicles = 0;
	while (invItr.hasNext()){
		CustomerInventory csrdb = invItr.next();
        ...
        <TR>
            ...
            <TABLE width="639">
                <TBODY>
                    ...
                    <TR>
                        <TD colspan="5" bgcolor="#cccccc" width="200">
                        <b>Dealership Inventory</b></TD>
                        <TD bgcolor="#cccccc" align="right" width="429">
                        <B>Number of Vehicles in Inventory: </B><%= numVehicles %></TD>
                    </TR>
                    <TR align="center">
...
<%
BigDecimal totalValue = SpecUtils.ZERO;
...
%>
            <TR bgcolor="#fafcb6" align="center">
                ...
                <TD><%= specUtils.formatNumber(SpecUtils.ONE.subtract(discountedPrice.divide(itemdb.getPrice(), 4, BigDecimal.ROUND_UP)).multiply(SpecUtils.HUNDRED).setScale(2, BigDecimal.ROUND_UP)/*itemdb.getDiscount().multiply(SpecUtils.onehundredBigDec)*/) %>%&nbsp;</TD>
...
{
     System.out.println("dealerinventory.jsp: error displaying user holdings");
}
				%>
                                <TR align="center">
                                    <TD></TD>
                        ...
                        <TABLE border="1" style="font-size: smaller" width="633">
                            <TBODY>
                                <TR align="center" bgcolor="#fafcb6">
                                    <TD colspan=2>Order ID: <%= odb.getId() %></TD>
                                    <TD colspan=2>Entry Date: <%= specUtils.formatDate(odb.getEntryDate()) %></TD>
                                    <TD colspan=2><A HREF=<%= "\"app?action=cancelorder&orderID="+odb.getId()+"\"" %>>cancel order</A>&nbsp;</TD>
                                </TR>
                                <TR bgcolor="#fafcb6">
                                	<TD colspan=6>Order Line Items</TD>
                                </TR>
                                <TR align="center">
...
try{
	...
	while (lineItr.hasNext()){
		OrderLine oldb = (OrderLine) lineItr.next();
		// only show orders that are waiting.
		if (oldb.getStatus() == OrderStatus.SHIPPED ) {
		  continue;
		}
		Item itemdb = oldb.getItem();
		BigDecimal itemPrice =
		  oldb.getTotalValue().divide(new BigDecimal(oldb.getQuantity()), 
...
catch (Exception e)
{
     System.out.println("dealerinventory.jsp: error displaying user holdings");
				...
				%>
                                <TR bgcolor="#fafcb6" align="center">
                                    <TD>&nbsp;</TD>
                                    <TD>Ord Status: <%= odb.getStatus() %></TD>
                                    <TD>&nbsp;</TD>
