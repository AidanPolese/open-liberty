 ...
 * --------------- ------ -------- --------------------------------------------
 * d412803         070102 djvines  Creation
 * d412959         070104 djvines  Use identityHashCode not hashCode
 * ============================================================================
 */
...
/* ************************************************************************** */
/**
 * A IntrospectionLevelMember holds the information about a single object to
 * be introspected.
 */
/* ************************************************************************** */
public final class IntrospectionLevelMember {

    /** OBJECT_TYPE is the name for the root object we're going to introspect deeper */
     ...
     * @param obj The other object to be tested
    ...
    /* -------------------------------------------------------------------------- */
    /*
     * IntrospectionLevelMember constructor
     * /* --------------------------------------------------------------------------
     */
    /**
    ...
    public IntrospectionLevelMember(Object member) {
        ...
        _allKnownMembersInThisTree.add(this); // Add ourselves to the all known members set
    }
     ...
     */
    /**
     * Construct a new IntrospectionLevelMember. NOTE: It's the caller's responsibility
     * to add this new member to any relevant allKnownMembers set
     * 
     * @param level The introspection level for this member
     * @param name The text used as the name of this level member
     * @param description The text used as the description of this level member
     * @param member The object being introspected
     * @param allKnownMembers The set of all known level members
     */
    private IntrospectionLevelMember(int level
                                     , String name
                                     , String description
                                     , Object member
     ...
     */
    /**
     * @return The set of child level members for this level member (excluding level members
     *         for objects already added by other level members)
     */
    public List<IntrospectionLevelMember> getChildren() {
        if (_children == null) {
            _children = new ArrayList<IntrospectionLevelMember>();
            if (_member != null) // A null member indicates that no further levels exist
            {
                ...
                if (memberClass.isArray()) {
                    ...
                    /*
                     * Loop around the fields of the object (including fields of its superclass)
                     * adding them as children (and, it we haven't seen the child before, getting
                     * introspected at the next level if its worth doing so)
                     */
     ...
     */
    /**
     * Add a new IntrospectionLevelMember to _children List (Checking to see if the
     * new child should be introspected further
     * 
    ...
    private Object getFieldValue(final Field field) {
        Object field_value = AccessController.doPrivileged(
     ...
     * 
     * @param currentClass The class to be introspected
     * @return the fields of the class
     */
    private Field[] getFields(final Class currentClass) {
        final Field[] objectFields = AccessController.doPrivileged(
     ...
     * 
     * @param value The value needing a description
     * @return The description
     */
    private String makeDescription(Object value) {
        String answer; // Not initialized, so the compiler tells if we miss a case
        if (value == null) {
            answer = "null";
    ...
    /* -------------------------------------------------------------------------- */
    /*
     * convertSimpleArrayToString method
     * /* --------------------------------------------------------------------------
     */
    /**
     ...
     */
    /**
     * Return either null (if the value doesn't need further introspection) or the
     * object reference if it does require further introspection
     * 
     * @param value The object that might require further introspection
     * @return null if the object doesn't require further introspection, value if it does
     */
    private Object makeMember(Object value) {
        Object answer = null; // Assume we don't require further introspection until proved otherwise

        if (value != null) {
            Class objClass = value.getClass();

            if (objClass.isArray()) {
                // Array's need further introspection if they're bigger than length 0
     ...
     * 
     * @param is The incident stream
     * @param maxDepth The maximum depth to descend
     */
