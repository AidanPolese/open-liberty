...
public class AlpineTracePreprocessInstrumentation extends AbstractInstrumentation {
    ...
    public final static Type ALPINE_TR_TYPE = AlpineTracingClassAdapter.TR_TYPE;
    public final static Type ALPINE_TRACE_COMPONENT_TYPE = AlpineTracingClassAdapter.TRACE_COMPONENT_TYPE;
    ...
    protected class ClassTraceInfo {
        ...
        // Explicitly declared Alpine TraceComponent
     ...
     */
    public AlpineTracePreprocessInstrumentation() {}
     ...
     */
    @SuppressWarnings("unchecked")
    private void processAlpineTraceComponentDiscovery(ClassTraceInfo info) {
        ...
        if (!traceComponentFields.isEmpty()) {
            ...
            if (traceComponentFields.size() > 0) {
                info.alpineTraceComponentFieldNode = traceComponentFields.get(0);
            }
        }
    }
    ...
    private void setupTraceStateObjectField(ClassTraceInfo info) {
        ...
        // Preference is given to the Alpine TraceComponent over the
        // WebSphere TraceComponent and either TraceComponent over a
        // Logger reference.  If none are declared, generate as a
        // synthetic.
        if (info.alpineTraceComponentFieldNode != null) {
            info.traceStateField = info.alpineTraceComponentFieldNode;
            info.traceStateFieldAlreadyInitialized = info.alpineTraceComponentFieldAlreadyInitialized;
    ...
    private boolean isMethodAlreadyInjectedAnnotationPresent(MethodNode methodNode) {
        ...
        if (injectedTraceAnnotation != null) {
            ...
            if (methodAdapters.contains(AlpineTracingMethodAdapter.class.getName())) {
                return true;
            }
        ...
        while (instructionIterator.hasNext()) {
            ...
            if (insnNode.getType() == AbstractInsnNode.FIELD_INSN) {
                ...
                if (fieldInsn.getOpcode() == Opcodes.PUTSTATIC) {
                    if (info.alpineTraceComponentFieldNode != null && fieldInsn.name.equals(info.alpineTraceComponentFieldNode.name)) {
                        info.alpineTraceComponentFieldAlreadyInitialized = true;
                    }
    ...
    protected byte[] transform(InputStream classfileStream) throws IOException {
        ...
        processAlpineTraceComponentDiscovery(info);
        ...
        if (injectStatic && ALPINE_TRACE_COMPONENT_TYPE.getDescriptor().equals(info.traceStateField.desc)) {
            cv = new AlpineTracingClassAdapter(cv, true);
        }

        // Pre-process the class and inject FFDC if requested
        if (info.traceStateField != null) {
            cv = new AlpineTracePreprocessClassAdapter(cv, !info.traceStateFieldAlreadyInitialized);
    ...
    public void processArguments(String[] args) throws IOException {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equalsIgnoreCase("--debug") || args[i].equals("-d")) {
                ...
                injectStatic = true;
            } else if (args[i].equalsIgnoreCase("--alpine")) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        AlpineTracePreprocessInstrumentation processor = new AlpineTracePreprocessInstrumentation();
